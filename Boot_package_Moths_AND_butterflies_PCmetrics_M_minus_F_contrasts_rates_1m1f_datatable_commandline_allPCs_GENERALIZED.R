...
if(length(args)==0){
  ...
  rdata<-'FINAL_PC_values_bootstrapping_overall_data_Moths_nocturnal_1m1f_AllPCs.RData'
  specimen_threshold=1
  rep=1
}else{
...
if(rdata!='No_Data'){
  ...
  if(Taxon_group=='Moths'){
    if(categorical_variable=='All'){
      PC_input<-read.csv('Nocturnal_Moths_1m1f_sensitivitycut_Final_PCA_scoreframe_sensitivity_curated_1MF_AllPCs.csv')
      PC_col_start=11
      PC_col_end=233
  ...
  if(Taxon_group=='Butterflies'){
    PC_input<-read.csv('Butterflies_1m1f_sensitivitycut_Final_PCA_scoreframe_sensitivity_curated_1MF_AllPCs.csv')
  ...
  if(Taxon_group=='Geometrids'){
    PC_input<-read.csv('Geometrids_1m1f_sens_cut_daynightOnly_Final_PCA_scoreframe_sensitivity_curated_1MF_AllPCs.csv')
    # prefix_str='Geometrids_BS_FINAL_allPCs'
    prefix_str='Geometrids_BS_FINAL_allPCs_timetree'
    IDvar<-'Geom_ID'
    PC_col_start=9
    PC_col_end=230
  }
  ...
  if(Taxon_group=='Butterflies'){
    ...
    drop=c(NULL)
    # names(PC_all)[names(PC_all) == 'Specimen_Barcode'] <- 'Barcode'
  }
  if(Taxon_group=='Geometrids'){
    drop=c(NULL)
  }
  ...
  if(Taxon_group=='Geometrids'){
    ...
    name_keys=read.csv('Geometridae_tips_from_Murillo_Ramos_etal_2019_Geometridae_08_04_2019_rooted_uraniidea_sematuridae_treefile_keyfile.csv')
    names(name_keys)[names(name_keys) == 'Geom_ID'] <- 'tip_ID'
    tree.pglmm<-read.tree("Geometridae_Murillo_Ramos2019_08_04_2019_rooted_uraniidae_sematuridae_timetree_correlated_lambda0_1_v2.newick")
    keep.tips.d<-unique(all_metrics_MDF1[paste('ID_var')][[1]])
    tree.pglmm.trimmed.d<-drop.tip(tree.pglmm,tree.pglmm$tip.label[-match(as.character(keep.tips.d), tree.pglmm$tip.label)])
  ...
  contributions<-function(M_metrics,F_metrics,metric_str,M_edgemat,F_edgemat,name_mat,edge_l){
    ...
    if(length(desc[!is.na(desc)])==0){
      ...
      rate_m<-rep(0,len)
      print(length(rate_m))
      print(M_edgemat[name_mat[,2],'edgename'])
  ...
  length(edgematrix_MDF1[name_matrix[, 2], "edgename"])
  PC_colnames
  ...
  rates<-merge(metadatafile,rates_combined_tips1,by.y='ID_var')
...
print("finished Abs Rate prop sexbias")
t2<-Sys.time()
t2-t1
...
fwrite(final_frame,paste('/path/to/outputfile/',rep,prefix_str,categorical_variable,'level_PC_metrics_Bootpkg_redo_rate_dimorphismSSQ_MD_secmomISdist',boot_reps2,'BS_CI_pvals',specimen_threshold,'M',specimen_threshold,'F.csv',sep='_'))
